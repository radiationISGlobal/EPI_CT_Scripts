...
output_filename <- "ec_res_leuk_ef_cumcat2_short_Fr.xlsx"
...
temp <- dt1_1[dt1_1$n_pe==0 & dt1_1$leuk_france==1,]
...
formula <- Surv(entry_age,exit_age,leuk_france)~loglin(factor(cumcat3_short))+strata(sex,country,birthcohort)

# fit the model
fit <- f_fit_linERR_all(formula,dt1_1,id_name="patientids",time_name="age",lag=lag)

# save results
results <- f_results_fit_2_cumcat2short_fr(fit, temp)
