...
dt1_1$cumcat3_short <- 1+
  ...
  as.numeric(dt1_1$dose_cum>=15)+
  as.numeric(dt1_1$dose_cum>=20)+
  as.numeric(dt1_1$dose_cum>=30)

lag <- 2

output_path     <- "analysis_2_11/results/"
output_filename <- "ec_res_leuk_ef_cumcat3_short_UK.xlsx"
outcome_name    <- "leuk_UK"
data_set_format <- "event_fromat"

# n cases by exp category
dt1_1$cumcat3_short <- as.factor(dt1_1$cumcat3_short)
temp <- dt1_1[dt1_1$n_pe==0 & dt1_1$leuk_UK==1,]
...
formula <- Surv(entry_age,exit_age,leuk_UK)~loglin(factor(cumcat3_short))+strata(sex,country,birthcohort)

# fit the model
fit <- f_fit_linERR_all(formula,dt1_1,id_name="patientids",time_name="age",lag=lag)

# save results
results <- f_results_fit_2_cumcat2short_uk(fit, temp)
