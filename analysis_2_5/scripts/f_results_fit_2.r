f_results_fit_2_cumcat2long <- function(fit)
{
  ...
  results[2:(n_outputs+1),7] <- rownames(attr(fit,"wald_ci"))
  results[2:(n_outputs+1),8] <- attr(fit,"wald_ci")[,1]
  results[2:(n_outputs+1),9] <- attr(fit,"loglin_coef")[,3]
  results[2:(n_outputs+1),10] <- attr(fit,"wald_ci")[,3]
  results[2:(n_outputs+1),11] <- attr(fit,"wald_ci")[,4]
  ...
  max_dose <- max(unlist(lapply(rsets,function(x) max(dt1$dose_cum[x]))))
  
  # mid points of intervals of categories of exposure
  x <- c(1,7.5,12.5,20,37.5,62.5,87.5,(max_dose-100)/2)
  # estimates for each interval
  y <- c(1,results[2:(n_outputs+1),8])
...
f_results_fit_2_cumcat2short <- function(fit)
{
  n_outputs <- nrow(attr(fit,"wald_ci"))
