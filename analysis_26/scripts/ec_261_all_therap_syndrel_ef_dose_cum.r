...
# ------------------------------
# Description:  epict analysis: linERR for all_excl_therap_syndrel dropping subjecst born before start cancer registry
# ------------------------------
...
# cases dropped
dropped <- dt1[dt1$all_excl_therap_syndrel==1 & year(dt1$birth)<dt1$startregy,]
table(dropped$country)

# drop subjects born before their respective cancer registries
dt1 <- dt1[year(dt1$birth)>=dt1$startregy,]

output_path     <- "analysis_26/results/"
output_filename <- "ec_res_all_excl_therap_syndrel_ef_dose_cum.csv"
