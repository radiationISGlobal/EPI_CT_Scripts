...
files <- list.files("analysis_29/results/",full.names = T)
y <- data.frame()
...
y$subset <- "Exclusion of 10 years"

o <- order(outcomes_order)
y <- y[order(o),]

write_xlsx(y,"results/epict_analysis_29_linERR_Excl10y_new_outcomesv2.xlsx")
