...
files <- list.files("analysis_28/results/",full.names = T)
y <- data.frame()
...
y$subset <- "Exclusion of 5 years"

o <- order(outcomes_order)
y <- y[order(o),]

write_xlsx(y,"results/epict_analysis_28_linERR_Excl5y_new_outcomesv2.xlsx")
