...
# ------------------------------
# Description:  epict analysis: linERR to subsets cohort AML_AL_ALMP_AL_excl_gen
...
source("analysis_3_2/scripts/f_results_fit_3.r")

dt <- readRDS("transformed_cohorts/dt1_new_outcomes_v2.rds")
lag <- 2

output_path     <- "analysis_3_2/results/"
output_filename <- "ec_res_AML_AL_ALMP_AL_excl_gen_ef_dose_cum_subsets.csv"
...
for(i in 1:length(subsets))
{
  ...
  formula <- Surv(entry_age,exit_age,AML_AL_ALMP_AL_excl_gen)~lin(dose_cum)+strata(sex,country,birthcohort)
