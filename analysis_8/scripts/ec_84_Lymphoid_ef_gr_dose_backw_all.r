...
# ------------------------------
# Description:  linERR to all cohort Lymphoid with grouped doses 0-5 / 5-10 / 10+ years before case occurs
# ------------------------------
...
output_filename <- "ec_res_Lymphoid_ef_gr_dose_backw_all.csv"
...
formula2 <- Surv(entry_age,exit_age,Lymphoid)~lin(cdose_0_4,cdose_5_9,cdose_10_)+strata(sex,country,birthcohort)
...
for(i in 1:length(rsets))
{
  ...
  #c("n_row","patientids","n_pe","entry_age","exit_age","Lymphoid","age","dose_cum","sex","country","birthcohort")
  ...
  dt <- select(dt,patientids,entry_age,exit_age,Lymphoid,age,ActMar_mean,n_pe,sex,country,birthcohort)
  ...
  dt <- dt %>% select(patientids,entry_age,exit_age,Lymphoid,sex,country,birthcohort)
  dt <- distinct(dt)
  ...
  aux <- aux[,c("n_row","patientids","n_pe","entry_age","exit_age","Lymphoid","age","cdose_0_4","cdose_5_9","cdose_10_","sex","country","birthcohort")]
...
cases <- unique(dt3[dt3$n_pe==0 & dt3$Lymphoid==1, c("patientids", "n_pe","cdose_0_4", "cdose_5_9", "cdose_10_")])
...
10302.56- 10305.6619708028 = 3.101971
  
1 - pchisq(3.101971, 2, lower.tail = T)
