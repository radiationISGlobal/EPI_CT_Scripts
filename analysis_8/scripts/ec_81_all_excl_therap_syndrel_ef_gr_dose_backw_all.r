...
# ------------------------------
# Description:  linERR to all cohort all_excl_therap_syndrel with grouped doses 0-5 / 5-10 / 10+ years before case occurs
# ------------------------------
...
output_filename <- "ec_res_all_excl_therap_syndrel_ef_gr_dose_backw_all.csv"
...
formula1 <- Surv(entry_age,exit_age,all_excl_therap_syndrel)~lin(dose_cum)+strata(sex,country,birthcohort)
# formula2: once cdose_0_4 cdose_5_9 cdose_10_ computed, fit the model
formula2 <- Surv(entry_age,exit_age,all_excl_therap_syndrel)~lin(cdose_0_4,cdose_5_9,cdose_10_)+strata(sex,country,birthcohort)
...
for(i in 1:length(rsets))
{
  ...
  #c("n_row","patientids","n_pe","entry_age","exit_age","all_excl_therap_syndrel","age","dose_cum","sex","country","birthcohort")
  # risk set i information
  row_id_case <- names(rsets)[i]
  id_case     <- dt2$patientids[as.numeric(row_id_case)] #id dels casos
  ft          <- unique(dt2$exit_age[which(dt2$patientids==id_case)]) #only cases exit age
  ids         <- dt1$patientids[rsets[[i]]] #ids dels controls
  
  # take the rows or colums of interest
  dt <- dt1[dt1$patientids %in% ids,]  #we take only controls
  dt <- select(dt,patientids,entry_age,exit_age,all_excl_therap_syndrel,age,ActMar_mean,n_pe,sex,country,birthcohort)
  
  # cut by the lag: take only exposures until ft-lag
  lag <- 2
  #dt  <- dt[which(dt$age < ft-lag),]
  dt[which(dt$age >= ft-lag), c("ActMar_mean")] <- 0
  ...
  dt_0_4 <- dt_0_4 %>% dplyr::group_by(patientids) %>% dplyr::summarize(cdose_0_4=sum(ActMar_mean))
  dt_5_9 <- dt_5_9 %>% dplyr::group_by(patientids) %>% dplyr::summarize(cdose_5_9=sum(ActMar_mean))  
  dt_10_ <- dt_10_ %>% dplyr::group_by(patientids) %>% dplyr::summarize(cdose_10_=sum(ActMar_mean))  
  ...
  dt <- dt %>% select(patientids,entry_age,exit_age,all_excl_therap_syndrel,sex,country,birthcohort)
  dt <- distinct(dt)
  ...
  aux <- aux[,c("n_row","patientids","n_pe","entry_age","exit_age","all_excl_therap_syndrel","age","cdose_0_4","cdose_5_9","cdose_10_","sex","country","birthcohort")]
  
  # add the row with the information of the case
  case_row <- which(aux$patientids==id_case)
  row_case <- aux[case_row,]
  row_case$n_pe<-0
  row_case$all_excl_therap_syndrel <- 1
...
cases <- unique(dt3[dt3$n_pe==0 & dt3$all_excl_therap_syndrel==1, c("patientids", "n_pe","cdose_0_4", "cdose_5_9", "cdose_10_")])
...
14117.2026074593 - 14111.89 = 5.312607

1 - pchisq(5.312607, 2, lower.tail = T)
