...
# ------------------------------
# Description:  epict analysis: linERR to subsets cohort NHL_Precursor_cell
# Appendix Table 4: Linear ERR/100 mGy by type of lymphoid neoplasm and by country - adjusted for age, sex, birth cohort
# ------------------------------
...
source("analysis_3_1/scripts/f_results_fit_3.r")

dt <- readRDS("transformed_cohorts/dt1_new_outcomes.rds")
lag <- 2

output_path     <- "analysis_3_1/results/"
output_filename <- "ec_res_NHL_Precursor_cell_ef_dose_cum_subsets.csv"
outcome_name    <- "NHL_Precursor_cell"
...
for(i in 1:length(subsets))
{
  ...
  formula <- Surv(entry_age,exit_age,NHL_Precursor_cell)~lin(dose_cum)+strata(sex,country,birthcohort)
